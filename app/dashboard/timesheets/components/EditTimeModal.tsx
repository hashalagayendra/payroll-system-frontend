...
export default function EditTimeModal({
  ...
  useEffect(() => {
    if (timesheet) {
      setFormData({
        ...
        work_date: timesheet.work_date ? new Date(timesheet.work_date).toISOString().split("T")[0] : "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axiosInstance.put(`/api/timesheets/${timesheet.id}`, formData);
      ...
      setError(
        ...
        "An error occurred while updating timesheet."
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center shrink-0">
          <h2 className="text-lg font-bold text-slate-800">Edit Time Entry</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto">
          ...
          <form id="edit-time-form" onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center gap-3 py-2">
              ...
              />
              <label htmlFor="edit_billable" className="text-sm font-medium text-slate-700 cursor-pointer select-none">
